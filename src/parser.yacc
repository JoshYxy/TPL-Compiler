...
LeftVal: ID DOT ID
{
  $$ = A_MemberExprLVal($1->pos, A_MemberExpr($1->pos, $1->u.id, $3->u.id));
}
| ID LBKT ID RBKT
{
  $$ = A_ArrExprLVal($1->pos,  A_ArrayExpr($1->pos, $1->u.id, A_IdIndexExpr($3->pos, $3->u.id)));
}
| ID LBKT NUM RBKT
{
  $$ = A_ArrExprLVal($1->pos, A_ArrayExpr($1->pos, $1->u.id, A_NumIndexExpr($3->pos, $3->u.num)));
}
| ID
{
...
| ID LBKT ID RBKT 
{
  $$ = A_ArrayExprUnit($1->pos, A_ArrayExpr($1->pos, $1->u.id, A_IdIndexExpr($3->pos, $3->u.id)));
}
| ID LBKT NUM RBKT 
{
  $$ = A_ArrayExprUnit($1->pos, A_ArrayExpr($1->pos, $1->u.id, A_NumIndexExpr($3->pos, $3->u.num)));
}
| ID DOT ID 
{
  $$ = A_MemberExprUnit($1->pos, A_MemberExpr($1->pos, $1->u.id, $3->u.id));
}
